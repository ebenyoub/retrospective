...
2026-07-20 (`T-ARCHI-01 — Migration des formulaires vers RHF` : sous-tâches 1/4 (`Login.tsx`, PR #29), 2/4 (`Signup.tsx`, PR #30) et 3/4 (`SessionCreate.tsx`) terminées, une branche/un commit par formulaire. Session de travail autonome, plan validé puis exécution déléguée jusqu'au bout, interruption uniquement en cas de problème réel.
- **Login.tsx / Signup.tsx** : remplacement de `useFormValidation` par `useForm`+`zodResolver`. `components/ui/FormField.tsx` n'étant pas `forwardRef` et exigeant une prop `value` contrôlée, résolu par `useWatch({ control, name })` + spread `{...register(...)}`. Sur Signup, découverte d'un bug de closure React dans l'ancien hook (clignotement d'un message d'erreur croisé sous le champ mot de passe lui-même à chaque frappe, jamais testé) — remonté et tranché explicitement par l'utilisateur avant commit : non reproduit, verrouillé par un nouveau test (`pages/auth/tests/Signup.test.tsx`, premier test du dossier).
