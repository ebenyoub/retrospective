...
2026-07-20 (`T-ARCHI-01 — Migration des formulaires vers RHF` : sous-tâche 1/4 (`Login.tsx`) terminée, sur `feature/T-ARCHI-01-login`. Session de travail autonome, l'utilisateur a validé un plan précis (analyse préalable de tous les formulaires du frontend, ordre du plus simple au plus risqué) puis délégué l'exécution jusqu'au bout, une branche/un commit par formulaire, en n'interrompant qu'en cas de problème réel.
...
- **Login.tsx** : remplacement de `useFormValidation` (hook maison) par `useForm` + `zodResolver`. Point technique clé : `components/ui/FormField.tsx` n'est pas `forwardRef` et exige une prop `value` contrôlée pour son état visuel (bordure verte/rouge) — `register()` seul ne la fournit pas. Résolu par `useWatch({ control, name })` (pas `watch()` classique, qui déclenche un avertissement React Compiler "fonction non memoïzable") combiné au spread `{...register(...)}`, sans toucher au composant partagé. Aucune nouvelle dépendance, périmètre limité à ce seul fichier.
- **Deux passes de revue `reviewer-code`** : la première a détecté une régression réelle (mot de passe composé uniquement d'espaces accepté silencieusement par `z.string().min(1,...)`, alors que l'ancien code le rejetait) et deux risques (`mode: "onChange"` ne couvrait pas le déclenchement sur `blur` seul de l'ancien code ; `.trim()` sur l'email changeait la valeur soumise). Corrigés : `.refine((v) => v.trim() !== "", ...)` sur les deux champs (ne transforme plus la valeur soumise), `mode: "all"` (blur + change), retrait d'un `disabled={isSubmitting}` ajouté par erreur sur le bouton (absent de l'original). Seconde revue : vérification empirique de l'ordre de priorité des deux `.refine()` chaînés sur l'email face au comportement réel de `zodResolver` (le message "requis" prime bien sur "format invalide", comme avant) — committable tel quel.
- **Vérification** : `tsc --noEmit` propre, `eslint` propre, 177/177 tests frontend (aucun test dédié à `Login.tsx` n'existe, seul filet de sécurité : `e2e/auth-journeys.spec.ts` test 1, passé contre la stack Docker réelle).)
