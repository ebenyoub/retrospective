...
- [ ] **AUDIT-05 — `guest_token` sans expiration propre** : partiellement daté (vérifié 2026-07-28) — `T-PART-02` a depuis ajouté une expiration à 24h depuis la jointure (`assertGuestTokenNotExpired`), et `US-14`/le correctif du 2026-07-28 (« permettre à un ancien invité de revoir une session close ») ont explicitement acté qu'un invité doit pouvoir consulter une session close en lecture seule — ce n'est donc plus un oubli mais un comportement voulu pour la lecture. Question encore ouverte : un jeton invité doit-il un jour cesser de fonctionner pour une session très ancienne (au-delà des 24h, sur une session jamais fermée mais expirée), ou le statu quo (24h + lecture seule illimitée si close) suffit-il ? Décision produit à trancher avec l'utilisateur si jugé prioritaire.
