...
export const signupSchema = z.object({
  body: z.object({
    username: z.string({ error: "Le pseudo est requis." }).min(4, "Le pseudo doit contenir au moins 4 caractères."),
    email: z.string({ error: "L'adresse email est requise." }).email("L'adresse email n'est pas valide."),
    password: z.string({ error: "Le mot de passe est requis." }).min(8, "Le mot de passe doit faire au moins 8 caractères.")
  })
});
